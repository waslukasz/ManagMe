...
export default function Functionality({data, updateState, updateHandler}: {data: FunctionalityType, updateState:any, updateHandler:any}) {
  ...
  const [functionalityData, setFunctionalityData] = useState<FunctionalityType>(currentFunctionality);
  ...
  async function editHandler() {
    ...
    if (!isEditing) return;
    setCurrentFunctionality(await functionalityApi.Update({...functionalityData, priority: parseInt(priorityRef.current?.value!), status: parseInt(statusRef.current?.value!) }));
    updateHandler();
  }
