...
export default function ProjectsCreate() {
  ...
  const [functionalityData, setFunctionalityData] = useState<FunctionalityDto>({name: '', description: '', priority: 0, project: projectApi.GetActiveId()!, status: 0, owner: ''});
  ...
  const handleSubmit = (event:React.MouseEvent) => {
    ...
    functionalityApi.Add({...functionalityData, priority: parseInt(priorityRef.current?.value!)});
      ...
      <div className='flex flex-col'>
        ...
        <div className="flex justify-center mt-7 font-sans">
          <form className="inline-flex flex-col gap-1">
            ...
            <input value={functionalityData.name} onChange={(event) => setFunctionalityData({...functionalityData, name: event.target.value})} className="rounded border border-solid p-2 border-black" type="text" />
            {createFailed &&
