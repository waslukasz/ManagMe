...
export default function Navigation() {
  ...
  async function handleLogin() {
    let tokens = await authApi.SignIn(usernameRef.current?.value!, passwordRef.current?.value!);
    if (tokens == null) {
      setLoginFailed(true);
      return;
    };
            ...
            <div className="relative z-50">
                ...
                { showLogin && 
                    <div className="text-black bg-white shadow absolute top-12 right-0 p-5 w-64 rounded font-sans">
                        ...
                        <input ref={usernameRef} type="text" name="username" id="username" placeholder='Username' className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow focus:shadow-gray-400 mt-1 mb-3' />
                        <label htmlFor="password" className='font-bold text-sm'>Password</label>
                        <input ref={passwordRef} type="password" name="password" id="password" placeholder='********' className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow focus:shadow-gray-400 mt-1' />
