...
export default function Functionality({data, updateState, updateHandler}: {data: FunctionalityType, updateState:any, updateHandler:any}) {
  ...
  const date = new Date(currentFunctionality.createdTimestamp)
    ...
    <div className="flex items-baseline gap-0.5 rounded-full justify-end mb-2">
        ...
        {!isEditing &&
            ...
            <div className="flex flex-col">
                ...
                <hr className='mt-3' />
                <span className='text-xs text-right italic'>{<>{date.getHours()}:{date.getMinutes()}:{date.getSeconds()} {date.getDay()}/{date.getMonth()+1}/{date.getFullYear()}</>}</span>
                <span className='text-xs text-right italic'>Priority: {currentFunctionality.priority == 0 && <>Low</>} {currentFunctionality.priority == 1 && <>Medium</>}{currentFunctionality.priority == 2 && <>High</>}</span>
